...
static CCamera::Frustum PyramidToFrustum(
	...
	assert(pyramid.GetVolumeType() == CollisionVolume::COLVOL_TYPE_PYRAMID);
...
TEST_CASE("CollisionHandler_PyramidVsCylinder + frustum tests")
{
	...
	const std::vector<Case> cases = {{
		...
		{
			"cam overhead, selection on left side",
			false,
			float3(12662.091f, 15218.569f, 34043.207f),
			{
				 0.963f, -0.271f,  0.001f, 0.000f,
				-0.000f,  0.004f,  1.000f, 0.000f,
				-0.271f, -0.963f,  0.004f, 0.000f,
			 -4609.448f, -15597.006f, 12350.267f, 1.000f
			}
		},
		{
		...
		// {
		// 	"fps cam, 90 fov, unit near edge, shouldnt hit",
		// 	false,
		// 	float3(35004.949, 5512.005, 22581.885),
		// 	{-0.35658f, -0.00279f, -0.93426f, 0.00000f,
		// 		0.00847f, -0.99996f, -0.00025f, 0.00000f,
		// 		-0.93423f, -0.00800f, 0.35659f, 0.00000f,
		// 		-10421.45508f, 531.83728f, 16087.72168f, 1.00000f}
		// },
	}};
	...
	for (const auto& tc : cases) {
		DYNAMIC_SECTION(tc.name) {
			...
			const bool hit = IntersectsVolume(pyramid, tc.pyramidMat, testVol, testMat);
			CHECK(hit == tc.intersects);
...
TEST_CASE("CollisionHandler_PyramidVsSphere")
{
	...
	const CollisionVolume testVol = [] {
		CollisionVolume v;
		v.InitShape(float3(46.000f, 46.000f, 46.000f), float3(0.000f, 0.000f, 0.000f), 4, 1, 2);
		return v;
	}();
	...
	const std::array<Case, 7> cases = {{
		{
			"outside corner",
			false,
			float3(3521.685f, 3665.413f, 32203.281f),
			{
				 0.99861f,  0.04203f,  0.03186f, 0.00000f,
				 0.00376f, -0.65935f,  0.75183f, 0.00000f,
				 0.05260f, -0.75066f, -0.65859f, 0.00000f,
			   862.04120f, -11239.06543f, 1869.90039f, 1.00000f
			}
		},
		{
			"outside top",
			false,
			float3(7471.025f, 3685.417f, 33070.977f),
			{
				 0.99938f, -0.03346f, -0.01050f, 0.00000f,
				 0.00494f, -0.16218f,  0.98675f, 0.00000f,
				-0.03472f, -0.98619f, -0.16192f, 0.00000f,
			  -560.03210f, -15401.72461f, 9621.08008f, 1.00000f
			}
		},
		{
			"touching top",

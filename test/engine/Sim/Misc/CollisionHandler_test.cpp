...
static bool IntersectsPyramid(
	const CollisionVolume& pyramidVol,
	const CMatrix44f& pyramidMat,
	const CollisionVolume& otherVol,
	const CMatrix44f& otherMat
) {
	return CCollisionHandler::IntersectPyramidVolume(&pyramidVol, pyramidMat, &otherVol, otherMat);
}
...
TEST_CASE("CollisionHandler_IntersectPyramidVolume_PyramidVsEllipse")
{
...
TEST_CASE("CollisionHandler_IntersectBoxVolume_BoxVsPyramid")
{
	...
	SECTION("containment near base") {
		...
		CHECK(Intersects(box, movedBoxMat, pyramidZ, pyramidMat));
	}
	...
	SECTION("separated beyond base plane") {
		...
		CHECK_FALSE(Intersects(box, movedBoxMat, pyramidZ, pyramidMat));
	}
	...
	SECTION("same lateral offset separates near apex but intersects near base") {
		...
		CHECK_FALSE(Intersects(box, MakeTransform(float3(1.3f, 0.0f, -1.0f)), pyramidZ, pyramidMat));

		// Near base: box spans z in [0.5, 1.5], pyramid half-width is at least 1.25.
		// Same x interval [0.8, 1.8] now overlaps.
		CHECK(Intersects(box, MakeTransform(float3(1.3f, 0.0f, 1.0f)), pyramidZ, pyramidMat));
	}
	...
	SECTION("sign-flipped primary axis is handled") {
		...
		CHECK(Intersects(box, MakeTransform(float3(1.3f, 0.0f, -1.0f)), pyramidZ, flippedPyramidMat));
		CHECK_FALSE(Intersects(box, MakeTransform(float3(1.3f, 0.0f,  1.0f)), pyramidZ, flippedPyramidMat));
	}
	...
	SECTION("non-Z primary axis") {
		...
		CHECK(Intersects(box, MakeTransform(float3(1.3f,  1.0f, 0.0f)), pyramidY, MakeTransform()));
		CHECK_FALSE(Intersects(box, MakeTransform(float3(1.3f, -1.0f, 0.0f)), pyramidY, MakeTransform()));
	}
	...
	SECTION("offset pyramid") {
		...
		CHECK(Intersects(box, MakeTransform(float3(4.3f, 0.0f, 1.0f)), offsetPyramid, MakeTransform()));
		CHECK_FALSE(Intersects(box, MakeTransform(float3(4.3f, 0.0f, -1.0f)), offsetPyramid, MakeTransform()));
	}
}
...
TEST_CASE("CollisionHandler_IntersectPyramidVolume_PyramidVsBox")
{
	...
	SECTION("box intersects near the wide end") {
		CHECK(IntersectsPyramid(pyramid, pyramidMat, box, MakeTransform(float3(1.3f, 0.0f, 1.0f))));
	}

	SECTION("same box placement separates near the apex") {
		CHECK_FALSE(IntersectsPyramid(pyramid, pyramidMat, box, MakeTransform(float3(1.3f, 0.0f, -1.0f))));
	}

	SECTION("touching apex counts as intersecting") {
		CHECK(IntersectsPyramid(pyramid, pyramidMat, box, MakeTransform(float3(0.0f, 0.0f, -2.5f))));
	}

	SECTION("sign-flipped pyramid primary axis is handled") {
		const CMatrix44f flippedPyramidMat = MakeTransform(ZeroVector, float3(0.0f, 2.0f * HALF_PI, 0.0f));

		CHECK(IntersectsPyramid(pyramid, flippedPyramidMat, box, MakeTransform(float3(1.3f, 0.0f, -1.0f))));
		CHECK_FALSE(IntersectsPyramid(pyramid, flippedPyramidMat, box, MakeTransform(float3(1.3f, 0.0f, 1.0f))));
	}
}
...
TEST_CASE("CollisionHandler_IntersectPyramidVolume_PyramidVsSphere")
{
	...
	SECTION("sphere contained") {
		...
		CHECK(IntersectsPyramid(pyramid, pyramidMat, sphere, MakeTransform(float3(0.0f, 0.0f, 0.0f))));
	}
...
TEST_CASE("CollisionHandler_IntersectPyramidVolume_PyramidVsEllipsoid")
{
	...
	SECTION("rotated ellipsoid still intersects when centered in pyramid") {
		...
		CHECK(IntersectsPyramid(pyramid, pyramidMat, ellipsoid, ellipsoidMat));
	}
}
...
TEST_CASE("CollisionHandler_IntersectPyramidVolume_PyramidVsCylinder")
{
	...
	SECTION("x-axis cylinder intersects near base but not near apex") {
		...
		CHECK(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform(float3(2.1f, 0.0f, 1.0f))));
		CHECK_FALSE(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform(float3(2.1f, 0.0f, -1.0f))));
	}
	...
	SECTION("z-axis cylinder centered at apex still intersects") {
		...
		CHECK(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform(float3(0.0f, 0.0f, -1.5f))));
	}
	...
	SECTION("z-axis cylinder near pyramid corner still intersects") {
		...
		CHECK(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform(float3(1.8f, 1.8f, 1.75f))));
	}

	SECTION("z-axis cylinder beyond pyramid corner is separated") {
		const CollisionVolume cylinder = MakeCylinderVolume(float3(0.75f, 0.75f, 1.0f), CollisionVolume::COLVOL_AXIS_Z);
		CHECK_FALSE(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform(float3(3.0f, 3.0f, 1.75f))));
	}
	...
	SECTION("large z-axis cylinder enclosing pyramid intersects") {
		...
		CHECK(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform()));
	}
	...
	SECTION("thin tall z-axis cylinder offset from axis misses pyramid") {
		const CollisionVolume cylinder = MakeCylinderVolume(float3(0.2f, 0.2f, 8.0f), CollisionVolume::COLVOL_AXIS_Z);
		CHECK_FALSE(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform(float3(3.0f, 0.0f, 0.0f))));
	}
	...
	SECTION("x-axis cylinder crossing the pyramid center intersects") {
		...
		CHECK(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform(float3(0.0f, 0.0f, 1.0f))));
	}

	SECTION("x-axis cylinder fully to the side is separated") {
		const CollisionVolume cylinder = MakeCylinderVolume(float3(3.0f, 0.5f, 0.5f), CollisionVolume::COLVOL_AXIS_X);
		CHECK_FALSE(IntersectsPyramid(pyramid, pyramidMat, cylinder, MakeTransform(float3(0.0f, 3.0f, 1.0f))));
	}
	...
	SECTION("pyramid transformed and cylinder still intersects in world space") {
		...
		CHECK(IntersectsPyramid(pyramid, movedPyramidMat, cylinder, cylMat));
	}
	...
	SECTION("pyramid transformed and cylinder separated in world space") {
		...
		CHECK_FALSE(IntersectsPyramid(pyramid, movedPyramidMat, cylinder, cylMat));
	}
}
TEST_CASE("CollisionHandler_IntersectPyramidVolume_PyramidVsPyramid")
{
	...
	SECTION("identical pyramids intersect") {
		CHECK(IntersectsPyramid(pyramidA, pyramidAMat, pyramidB, MakeTransform()));
	}

	SECTION("base-to-apex touching counts as intersecting") {
		// A base plane is at z=+2, B apex is at z=+2 when centered at z=4.
		CHECK(IntersectsPyramid(pyramidA, pyramidAMat, pyramidB, MakeTransform(float3(0.0f, 0.0f, 4.0f))));
	}

	SECTION("separated along primary axis") {
		CHECK_FALSE(IntersectsPyramid(pyramidA, pyramidAMat, pyramidB, MakeTransform(float3(0.0f, 0.0f, 4.1f))));
	}
	...
	SECTION("oppositely oriented pyramids overlap") {
		...
		CHECK(IntersectsPyramid(pyramidA, pyramidAMat, pyramidB, pyramidBMat));
	}

	SECTION("offset pyramids separate laterally") {
		CHECK_FALSE(IntersectsPyramid(pyramidA, pyramidAMat, pyramidB, MakeTransform(float3(5.0f, 0.0f, 0.0f))));
	}
}
